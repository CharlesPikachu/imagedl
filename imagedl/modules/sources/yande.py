'''
Function:
    Implementation of YandeImageClient
Author:
    Zhenchao Jin
...
'''YandeImageClient'''
class YandeImageClient(BaseImageClient):
    source = 'YandeImageClient'
    def __init__(self, **kwargs):
        super(YandeImageClient, self).__init__(**kwargs)
    ...
    def _parsesearchresult(self, search_result: str) -> list[ImageInfo]:
        ...
        for item in search_result:
            ...
            candidate_urls = [item.get('file_url'), item.get('jpeg_url'), item.get('sample_url'), item.get('preview_url')]
    ...
    def _constructsearchurls(self, keyword: str, search_limits: int = 1000, filters: dict = None, request_overrides: dict = None):
        request_overrides, filters, base_url = request_overrides or {}, filters or {}, "https://yande.re/post.json?"
        (params := {"tags": f"{keyword}", "page": 1, "limit": 50}).update(filters)
